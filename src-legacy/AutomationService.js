...
class AutomationService {
  ...
  /**
   * Setup token approvals for all tokens in a vault across all target platforms
   * Approvals are set to each platform's designated router (e.g., Universal Router for Uniswap)
   * @memberof module:AutomationService~AutomationService
  ...
  async setupTokenApprovals(vault) {
    // Validate target platforms exist - never default to a platform
    if (!vault.targetPlatforms?.length) {
      throw new Error(`Vault ${vault.address} has no target platforms configured`);
    }

    // Get all unique tokens: vault tokens + target tokens + position tokens (if any exist)
    const vaultTokenSymbols = Object.keys(vault.tokens || {});
    const targetTokenSymbols = vault.targetTokens || [];

    // Extract tokens from existing positions via pool data (may be empty during initial setup)
    ...
    if (allTokenSymbols.length === 0) {
      throw new Error(`Vault ${vault.address} has no tokens to approve`);
    }

    this.log(`Setting up token approvals for ${allTokenSymbols.length} tokens across ${vault.targetPlatforms.length} platform(s) for vault ${vault.address}`);

    // Track approvals needed per platform
    const approvalsByPlatform = new Map();
    ...
    for (const platformId of vault.targetPlatforms) {
      ...
      // Check each token for this platform's approval target
      for (const tokenSymbol of allTokenSymbols) {
        ...
        // Check if approval already exists for this platform's router
      ...
      if (approvalsNeeded.length > 0) {
        approvalsByPlatform.set(platformId, { approvalTarget, approvalsNeeded });
      }
    }

    // Execute all approvals with retry logic for transient failures
    let totalApprovals = 0;
    for (const [platformId, { approvalTarget, approvalsNeeded }] of approvalsByPlatform) {
      this.log(`Executing ${approvalsNeeded.length} token approvals for vault ${vault.address} to ${platformId} router (${approvalTarget})`);

      for (const { tokenAddress, tokenSymbol } of approvalsNeeded) {
        await retryWithBackoff(
          () => this.executeTokenApproval(vault.address, tokenAddress, tokenSymbol, approvalTarget),
          {
            maxRetries: 2,
            baseDelay: 1000,
            exponential: true,
            context: `Token approval for ${tokenSymbol} to ${platformId}`,
            logger: this
          }
        );
        totalApprovals++;
    ...
    if (totalApprovals > 0) {
      this.log(`Token approvals complete for vault ${vault.address}: ${totalApprovals} approval(s) across ${approvalsByPlatform.size} platform(s)`);
    } else {
