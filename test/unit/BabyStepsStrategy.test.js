...
describe("BabyStepsStrategy", function () {
  ...
  describe("Customization Override", function () {
    it("Should return custom values over template values", async function () {
      ...
      await executeOnStrategy(vault1Contract, user1, "setRangeParameters", [customValue, 1000, 100, 100]);
  ...
  describe("getAllParameters", function () {
    ...
    const PARAM_TYPES = ['uint16', 'uint16', 'uint16', 'uint16', 'bool', 'uint256', 'uint16', 'uint16', 'uint16', 'uint16'];
    ...
    it("Should return bytes that can be decoded to all parameters", async function () {
      ...
      // Verify all 10 parameters are returned in correct order
      ...
      expect(params[2]).to.equal(MOD_REBALANCE_THRESHOLD_UPPER);// rebalanceThresholdUpper
      expect(params[3]).to.equal(MOD_REBALANCE_THRESHOLD_LOWER);// rebalanceThresholdLower
      expect(params[4]).to.equal(MOD_FEE_REINVESTMENT);         // feeReinvestment
      expect(params[5]).to.equal(MOD_REINVESTMENT_TRIGGER);     // reinvestmentTrigger
      expect(params[6]).to.equal(MOD_REINVESTMENT_RATIO);       // reinvestmentRatio
      expect(params[7]).to.equal(MOD_MAX_SLIPPAGE);             // maxSlippage
      expect(params[8]).to.equal(MOD_EMERGENCY_EXIT_TRIGGER);   // emergencyExitTrigger
      expect(params[9]).to.equal(MOD_MAX_UTILIZATION);          // maxUtilization
    });
    ...
    it("Should include custom values mixed with template values", async function () {
      ...
      // Custom values
      expect(params[7]).to.equal(customSlippage);
      expect(params[8]).to.equal(1500);
      expect(params[9]).to.equal(8000);
    });
