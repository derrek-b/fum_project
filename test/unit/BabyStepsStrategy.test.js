...
describe("BabyStepsStrategy", function () {
  ...
  const CONS_TARGET_RANGE_UPPER = 1000;
  const MOD_TARGET_RANGE_UPPER = 500;
  const AGG_TARGET_RANGE_UPPER = 300;
  const STBL_TARGET_RANGE_UPPER = 50;

  const CONS_REBALANCE_THRESHOLD_UPPER = 300;
  const MOD_MAX_SLIPPAGE = 50;
  const AGG_REINVESTMENT_RATIO = 10000;
  const STBL_EMERGENCY_EXIT_TRIGGER = 200;
  ...
  describe("Parameter Getters", function () {
    it("Should return moderate template values by default", async function () {
      ...
      const maxSlippage = await strategy.getMaxSlippage(vault1.address);
      expect(maxSlippage).to.equal(MOD_MAX_SLIPPAGE);
    });
    ...
    it("Should return conservative template values when selected", async function () {
      ...
      // Conservative doesn't do fee reinvestment
      const feeReinvestment = await strategy.getFeeReinvestment(vault1.address);
      expect(feeReinvestment).to.equal(false);
    });
    ...
    it("Should return aggressive template values when selected", async function () {
      ...
      const reinvestmentRatio = await strategy.getReinvestmentRatio(vault1.address);
      expect(reinvestmentRatio).to.equal(AGG_REINVESTMENT_RATIO);
    });
    ...
    it("Should return stablecoin template values when selected", async function () {
      ...
      const rangeUpper = await strategy.getTargetRangeUpper(vault1.address);
      expect(rangeUpper).to.equal(STBL_TARGET_RANGE_UPPER);

      const emergencyExitTrigger = await strategy.getEmergencyExitTrigger(vault1.address);
      expect(emergencyExitTrigger).to.equal(STBL_EMERGENCY_EXIT_TRIGGER);
    });
